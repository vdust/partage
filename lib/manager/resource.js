...
function initRef(resource) {
  ...
  resource.ref = function (callback) {
    if (ref < 0) {
      throw new Error("Resource expired ["+this.+"]");
    }
    ref++;
...
proto.stat = function stat(callback) {
  ...
  if (typeof callback === 'function') {
    resource.once('stat', this.ref(callback));
  }
  if (resource._statPending) return;
  resource._statPending = true;

  fs.stat(this.abspath, function (err, stats) {
    delete resource._statPending;

    if (err) {
      delete resource.stats;
      return resource.emit('stat', Resource.createError(err, 'resource'));
    }

    resource.stats = freeze({
      mtime: stats.mtime,
      type: stats.isDirectory() ? 'folder' : 'file',
      mime: stats.isFile() ? mime.lookup(this.name) : 'inode/directory'
    });

    resource.emit('stat', null, resource.stats);
  });
};
...
proto.scan = function scan(callback) {
  ...
  if (callback) this.on('scan', callback);
  ...
  fs.readdir(this.abspath, function (err, all) {
    if (err) {
      ...
      return resource.emit('scan', Resource.createError(err));
    }

    var dirs = [];
    var files = [];

    function add(f, stats) {
      var isDir = stats.isDirectory();
      var list = isDir ? dirs : files;
      var obj = {
        name: f,
        type: isDir ? 'folder' : 'file',
        mime: isDir ? 'inode/directory' : mime.lookup(f),
        mtime: stats.mtime
      };

      if (!isDir) obj.size = stats.size;

      list.push(freeze(obj));
    }

    async.eachLimit(all, Resource.asyncLimit, function statFile(f, next) {
      if (f[0] === '.') return process.nextTick(next);
      fs.stat(resolve(resource.abspath, f), function (err, stats) {
        if (err) {
          console.log("scan %s/%s: %s", resource.abspath, f, err.toString());
...
proto.mkdir = function (options, callback) {
  ...
  if (typeof callback === 'function') {
    resource.once('mkdir', this.ref(callback));
  }
  ...
  this.stat(function (err, stats) {
    ...
    if (err && err.error && err.error.code === 'ENOENT') {
      ...
      return fn(resource.abspath, function (err) {
        ...
        resource.emit('mkdir', err ? resource.createError(err) : null);
      });
    }

    delete resource._mkdirPending;
    resource.emit('mkdir', err);
  });
};
...
Resource.checkName = function checkName(path, errMerge) {
  var ctx = typeof this === 'string' ? this : 'resource.';
  var r = /^[^\\\/:\0-\u001f\u007f. ][^\\\/:\0-\u001f\u007f]*$/.test(name);

  if (!r) {
    throw error.createError(400, ctx+'name',
      "Resource name contains illegal characters",
      utils.merge({
        minLength: 1,
        allowedFirst: "[^\\\\/:\\0-\\u001f\\u007f. ]",
        allowed: "[^\\\\/:\\0-\\u001f\\u007f]"
...
Resource.checkPath = function checkPath(path, errMerge) {
  ...
  if (!rpath) {
    ...
    for (var i = 0; i < rpath.length; i++) {
      Resource.checkName.call(ctx, rpath[i], utils.merge({ index : i }, errMerge));
    }
  }
};
...
Resource.createError = function createError(err, stackOpt) {
  ...
  if (error.isLibError(err)) return err;
  ...
  var error = new error.LibError(statusCode, errorCode, message, {}, stackOpt);

  Object.defineProperty(error, 'error', { value: err });

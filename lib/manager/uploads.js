...
proto.save = function save(req, callback) {
  ...
  do {
    tmpFile = resolve(this.path, Uploads.tmpName());
    ...
    try {
      ...
      if (e.code !== 'EEXIST') {
        console.log(e);
        return done(error(500, "Unexpected Error", {
          code: 'unexpected',
        }));
      }
    }
  ...
  function onAborted() {
    ...
    done(utils.error(400, "Request aborted", { code: 'request.aborted' }));
  }
  ...
  function onData(chunk) {
    ...
    if (limit !== null && received > limit) {
      done(utils.error(413, "Request entity too large", {
  ...
  function onEnd(err) {
    ...
    if (length !== null && received !== length) {
      done(utils.error(400, "Request size did not match content length", {

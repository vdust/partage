...
proto.loadUsers = function loadUsers(callback) {
  ...
  this.lock('__usersfile', function (release) {
    fs.readFile(this._usersfile, 'utf8', function (err, data) {
      if (!err) {
...
proto.saveUsers = function saveUsers(callback) {
  ...
  if (this._ctl.usersFileProtected) {
    var error = new Error("Can't save users file. Contact a server administrator.");
    error.code = 'EPERM';
  ...
  var users = Object.keys(this.users).sort();
  var rows = [];
  for (var i = 0; i < users.length; i++) {
    rows.push(this.users[users[i]].csv());
  }
  ...
  this.lock('__usersfile', true, function (release) {
    function _done(err) { release(); done(err); }
    fs.copy(this._usersfile, this._usersfile+'-', function (err) {
      if (err && err.code !== 'ENOENT') {
        console.error("Failed to backup %s: %s", manager._usersfile, e.toString());
        return done(err);
      }

      fs.writeFile(self._usersfile, rows, 'utf8', function (err) {
        if (err) {
...
proto.user = function user(username, create) {
  var u = this.users[username];

  if (!u && create) u = this.users[username] = new User(username);

  return u;
};

proto.userDelete = function userDelete(user) {
...
proto.loadFolders = function loadFolders(callback) {
  ...
  fs.readdir(this.root, function (err, files) {
    ...
    async.eachLimit(files, Resource.asyncLimit, function statDir(f, next) {
      ...
      var path = resolve(manager.root, f);
...
proto.folder = function folder(foldername, create, infos) {
  ...
  if (!f && typeof create === 'function') {
    nf = new Folder(resolve(this.root, foldername));
...
Manager.superUser = new User('__su__::su:0'); /* for internal use */

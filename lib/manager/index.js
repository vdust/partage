...
proto.folderRegister = function folderRegister(folder) {
  ...
  folder.on('trash', function () {
    delete manager.folders[folder.name];
  });

  folder.on('rename', function (oldName, newName) {
    delete manager.folders[oldName];
    manager.folders[newName] = folder;
  });
};
...
proto.rename = function (user, src, dest, replace) {
  var manager = this;

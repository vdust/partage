...
proto._create = function _create(nameOrRow) {
  ...
  var passhash = row[1]||'';
  var level = Access.level(row[2]) || Access.LEVELS.user;
  var disabled = !!+row[3];
  var email = row[4] || '';
  ...
  // trick for dev bootstrapping
  if (typeof passhash === 'function') {
    passhash(this);
  }
};
...
proto.setLevel = function setLevel(lvl, limit) {
  console.warn("Deprecated User.setLevel()");
...
User.checkName = function checkName(name, errMerge) {
  ...
  if (!User.USERNAME_RE.test(name)) {
    ...
    throw createError(400, ctx+'username.'+code, msg, merge(extra, errMerge));
  }
};
...
User.checkPassword = function checkPassword(pwd, errMerge) {
  ...
  if (code) throw createError(400, ctx+'password.'+code, msg, merge(extra, errMerge));
};

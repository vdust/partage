    ...
    _ctlClean: {
      value: function () {
        for (var i = 0; i < arguments.length; i++) {
          delete folder._ctl[arguments[i]];
        }
      }
    },
    _lock: { value: new rwlock() },
...
proto.lock = function lock(level, callback) {
  ...
  if (level !== 'read' && level !== 'write') {
    throw new Error("Lock level must be 'read' or 'write'");
  }
  if (typeof callback !== 'function') {
    throw new Error("Callback must be a function, got " + typeof callback);
  }

  this._lock[level+'Lock'](callback);
};
...
proto.pathRelative = function pathRelative() {
  ...
  fp = this.abspath;
  p = this.getPath.apply(this, Array.prototype.slice.call(arguments, offset));
...
proto.configure = function setInfos(conf, all) {
  var modified = false;
  ...
  if (all || 'access' in conf) {
    this.access = Folder.cleanAccess(conf.access);
    modified = true;
  }
  ...
  if (modified) this._synced = false;
  return modified;
};
...
proto.loadConfig = function loadConfig(callback) {
  ...
  if (typeof callback === 'function') {
    folder.once('loadconfig', callback);
  }

  if (this._ctl._loadConfig) return;
  this._ctl._loadConfig = true;

  function done(err) {
    if (err) {
      folder._ctlClean('synced');
    } else {
      folder._ctl.synced = true;
    }
    folder._ctlClean('_loadConfig');
    folder.emit('loadconfig', err);
  }

  this._.stat(function (err, stats) {
    if (err) {
      folder._ctlClean('exist');
      return done(err);
    }

    folder._ctl.exist = true;
    ...
    fs.ReadFile(folder._configFile(), 'utf8', function (err, data) {
      ...
      if (data) {
        try {
          ...
          // Don't fail. Just ignore bad conf.
          console.warn(this._configFile()+": "+e.toString());
        }
      }

      folder.configure(true, conf); // clear all
...
proto.saveConfig = function saveConfig(callback) {
  ...
  if (typeof callback === 'function') {
    this.once('saveconfig_schedule', function () {
      folder.once('saveconfig', callback);
    });
  }

  if (this._ctl._saveConfigPending) return;

  // To preserve configuration in sync, we need to use a double queue
  // in case a save operation is in progress, the data to write being
  // frozen before internal async calls are performed.
  if (this._ctl._saveConfig) {
    this._ctl._saveConfigPending = true;

    this.once('saveconfig', function () {
      folder._ctlClean('_saveConfigPending');
      folder.saveConfig();
    });
  ...
  this._ctl._saveConfig = true;

  this.emit('saveconfig_schedule');

  function done(err) {
    folder._ctlClean('_saveConfig');

    if (err || this._ctl._saveConfigPending) {
      folder._ctlClean('synced');
    } else {
      folder._ctl.synced = true;
    }

    folder.emit('saveconfig', err);
  }
  ...
  fs.copy(conffile, conffile+'-', { clobber: true }, function (err) {
    ...
    fs.writeFile(conffile, data, 'utf8', function (err) {
      done(err && Resource.createError(err));
    });
  });
};
...
proto.save = function save(callback) {
  ...
  if (typeof callback === 'function') {
    this.once('save', callback);
  }

  if (this._ctl._save) return;
  this._ctl._save = true;

  function done(err) {
    folder._ctlClean('_save');
    folder.emit('save', err);
  }
...
proto.rename = function rename(newname, callback) {
  ...
  if (this._ctl._rename) {
    console.warn("Concurrent rename detected. Must sync with write lock explicitly.");
...
Folder.checkAccessList = function checkAccessList(access, errMerge) {
  ...
  if (Array.isArray(access)) {
    ...
    keys = Object.keys(access);
    for (i = 0; i < keys.length; i++) {
      errDetails.key = key[i];
      User.checkName.call(ctx+'object.key.', key[i], errDetails);

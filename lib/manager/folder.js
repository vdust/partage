...
proto._create = function _create(options, path, callback) {
  var folder = this;
  var resource;
  var opts = { read: false, write: false, trustedName: false };

  if (typeof options === 'string' || Array.isArray(options)) {
    callback = path;
    path = options;
  } else if (typeof options === 'boolean') {
    opts.trustedName = options;
  } else if (typeof options === 'object') {
    utils.merge(opts, options);
  }

  if (Array.isArray(path)) {
    path = resolve.apply(null, path);
  } else if (path[0] !== '/') {
    path = resolve(path);
  }

  var name = opts.trustedName ? basename(path) : Resource.sanitizeName(basename(path));
...
proto._configFile = function _configFile(ext) {
  return joinPath(this.abspath, '.ptconfig'+(ext||''));
};
...
proto.can = function can(can, user) {
  ...
  switch (can.toLowerCase()) {
    case 'read':
      return this._options.read || this.accessList.hasOwnProperty(user.name);
    case 'write':
      return this._options.write || this.accessList[user.name] === 'rw';
    default:
...
proto.resource = function resource(path) {
  ...
  if (!(relpath in this._resources)) {
    ...
    Object.defineProperty(resource, '_releasecb', {
      enumerable: false,
      writable: false,
      value: function () {
    ...
    resource.once('release', resource._releasecb);
  }

  return this._resources[relpath].ref();
};

// Called at the time a folder is unregistered to prevent concurrency
// side effects.
proto._cleanResources = function _cleanResources() {
  var relpath, r;
  for (relpath in this._resources) {
    if (relpath === '.') continue; // Don't clear root resource.
    r = this._resources[relpath];
    delete this._resources[relpath];
    r.removeListener(r._releasecb);
    // Remove the release callback set at insertion time.
  }
};

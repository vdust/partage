...
proto._create = function _create(path, callback) {
  ...
  this._.on('rename', function (newRes) {
    ...
    // In this case, we have another folder instance in newRes.
    // We must replace the mock/removred folder instance by this
    // one for newRes to remain valid.
    Object.defineProperty(newRes, 'folder', {
      enumerable: true,
      writable: false,
      value: folder
    });

    folder.emit('rename', oldName, newRes.name);
  });

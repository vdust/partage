...
Router.route = function route(path) {
  ...
  if (pathKeys.length) {
    ...
    if (this.mergeParams) {
      ...
      // just in case.
      console.log("WARNING: Router uses mergeParams = true, but the Router.route() override for path-to-regexp 1.2.x will not merge numeric params correctly if any appear in the parent.");
    }

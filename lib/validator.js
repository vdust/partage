...
function validateType(v, expect, errMerge) {
  ...
  function _err(msg, extra) {
    ...
    return createError(400, ctx+'type', msg, infos, _err);
  };
  ...
  switch (type) {
    ...
    case 'enum':
      if (typeof v !== 'string') {
        throw _err("enum key (string) expected, got " + typeof v);
      }
      if (Array.isArray(expect['enum'])) {
        if (!expect['enum'].filter((e) => e === v).length) {
          throw createError(400, ctx+'value', "Unknown enum value '"+v+"'", {
            expect: expect['enum'],
            got: v
          });
        }
